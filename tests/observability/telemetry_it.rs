use clustor::observability::telemetry::{
    metrics_bucket_catalog, IncidentCorrelator, IncidentDecision, MetricsCursor, MetricsRegistry,
...
fn registry_enforces_namespace_and_histograms() {
    ...
    let histogram = registry.register_histogram("latency_ns", 1_000);
    histogram.observe(123.0);
    registry
        .observe_histogram("latency_ns", 450.0)
        .expect("histogram observed");
    let snapshot = registry.snapshot();
    match snapshot.histograms.get("clustor.latency_ns") {
        Some(counts) => assert!(!counts.is_empty()),
        None => panic!("latency histogram missing from snapshot"),
    }
...
fn metrics_catalog_exposes_artifact_buckets() {
    let catalog = metrics_bucket_catalog().expect("catalog loads");
