...
fn activation_checkpoint_digest_matches_readiness_feed() {
    ...
    let digest = readiness_digest(&snapshot.records);
    let decision = ActivationBarrierEvaluator::evaluate(&barrier(), &snapshot.records, 41_000);
...
fn activation_checkpoint_reports_pending_state_and_digest() {
    ...
    let decision = ActivationBarrierEvaluator::evaluate(&barrier(), &snapshot.records, 15_000);
    ...
    assert_eq!(
        ...
        readiness_digest(&snapshot.records)
    );
}
