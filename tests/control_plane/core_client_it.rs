use clustor::control_plane::core::client::{
    CpApiTransport, CpClientError, CpControlPlaneClient, TransportError, TransportResponse,
};
use clustor::control_plane::core::CpPlacementClient;
use clustor::feature_guard::FeatureManifest;
...
fn client_identity(now: Instant) -> MtlsIdentityManager {
    ...
    MtlsIdentityManager::new(cert, "cp.internal", Duration::from_secs(600), now)
}
...
impl CpApiTransport for MockTransport {
    fn get(&self, path: &str) -> Result<TransportResponse, CpClientError> {
        self.responses
            .get(path)
            .cloned()
            .ok_or_else(|| CpClientError::Transport(TransportError::NoRoute { path: path.into() }))
    }
}
