...
impl<T: Copy + Default, const N: usize> RingBuf<T, N> {
    /// Create an empty ring buffer. The backing storage is initialised
    /// via `T::default()` rather than `mem::zeroed`, so types whose
    /// `Default` differs from the all-zero bit pattern (e.g. `NonZeroU64`)
    /// stay safe. Not a `const fn` because `[T::default(); N]` is not
    /// `const`-callable for generic `T` on stable Rust.
    pub fn new() -> Self {
        Self {
            buf: [T::default(); N],
            head: 0,
