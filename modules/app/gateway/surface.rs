...
use super::{codec, throttle, wire, wire_channels};

#[repr(C)]
pub struct Surface {
    pub in_requests: i32,        // in: cleartext MSG_CLIENT_FRAME from peer_router
    pub in_throttle_status: i32, // in: ThrottleEnvelope from admission
...
/// Per-step bound: ≤8 inbound frames routed (each at most one
/// codec/throttle hand-off), ≤4 admin responses forwarded, ≤4 frames
/// drained from each diagnostic input.
///
/// # Safety
///
...
pub unsafe fn step(
    ...
    if su.in_requests >= 0 {
        for _ in 0..8 {
            ...
            let (_frame_type, plen) =
                wire_channels::channel_read_msg(sys, su.in_requests, &mut su.msg_buf);
            if plen < 4 {
                continue; // need at least conn_id + 3-byte envelope header
            }
            ...
            match msg_type {
                ...
                wire::MSG_ADMIN_COMMAND => {
                    if su.out_admin_req >= 0 {
                        ...
                        if poll_out > 0 && (poll_out as u32 & 0x02) != 0 {
                            ...
                            framed[0] = conn_id;
                            let pl = (payload_end - payload_start).min(framed.len() - 1);
                            framed[1..1 + pl]
                ...
                _ => {
                    ...
                    framed[1..1 + pl]
                        .copy_from_slice(&su.msg_buf[payload_start..payload_start + pl]);
                    codec::on_request(co, th, su, sys, routed_type, &framed[..1 + pl]);
                    su.requests_routed += 1;
                }
            }
        }
    ...
    // 2. Forward admin responses → peer_router. Client responses
    //    return through send_response calls from the codec.
    ...
    //    is future work)
    drain_input(sys, su.in_throttle_status, &mut su.msg_buf);
