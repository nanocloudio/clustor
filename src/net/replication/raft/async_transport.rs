...
impl AsyncClientSession {
    async fn connect(config: &AsyncSessionPoolConfig, now: Instant) -> Result<Self, NetError> {
        ...
        let stream = with_timeout(config.connect_timeout, TcpStream::connect(target.clone()))
            .await
    ...
    pub async fn send(
        ...
        with_timeout(io_timeout, io_fut)
            .await
...
async fn handle_connection<H: RaftRpcHandler>(
    ...
    let mut tls_stream = with_timeout(Duration::from_secs(5), acceptor.accept(stream))
        .await
