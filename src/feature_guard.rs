...
impl ParkedFeatureGate {
    ...
    #[cfg(test)]
    pub fn with_manifest_override(
        manifest: FeatureManifest,
        verifying_key_hex: &str,
        enabled: bool,
    ) -> Result<Self, FeatureManifestError> {
        Self::build_gate(manifest, verifying_key_hex, enabled)
    }
...
mod tests {
    ...
    fn gate_blocks_when_env_missing() {
        let (manifest, verifying_hex) = sample_manifest();
        let mut gate =
            ParkedFeatureGate::with_manifest_override(manifest, &verifying_hex, false).unwrap();
        assert!(gate
    ...
    fn gate_allows_when_env_enabled() {
        let (manifest, verifying_hex) = sample_manifest();
        let mut gate =
            ParkedFeatureGate::with_manifest_override(manifest, &verifying_hex, true).unwrap();
    ...
    fn gate_rejects_manifest_with_bad_signature() {
        ...
        let wrong_hex = hex::encode(wrong_key.verifying_key().to_bytes());
        assert!(ParkedFeatureGate::with_manifest_override(manifest, &wrong_hex, true).is_err());
    }
}
